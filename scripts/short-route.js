...
function findPathBetweenSchools(addressData, jcts, stJcts, beelines, start, end) {
    const [startName, startType, startCampus] = splitSchoolDescription(start);
    const [endName, endType, endCampus] = splitSchoolDescription(end);
    const startSchool = findSchool(schoolData, startName, startType, startCampus);
    if (!startSchool) {
        console.log('Cannot find school:', start);
    }
    const endSchool = findSchool(schoolData, endName, endType, endCampus);
    if (!endSchool) {
...
findPathBetweenSchools(addressData, jcts, stJcts, beelines, 'Las Americas Early Education', 'Moscone Elementary');
...
findPathBetweenSchools(addressData, jcts, stJcts, beelines, 'Rooftop Elementary - Twin Peaks', 'Rooftop Elementary - Mayeda');
