...
/**
 * Encode form input for safe output to HTML and/or a URL.
 *
 * @param {string} value - Unescaped user input
 * @param {string} [url=false] - Whether to use "+" for spaces.
 * @returns {string} The input string with unsafe characters escaped
 */
export function encodeInput(value, url = false) {
    const space = url ? '+' : ' ';
    return encodeURIComponent(value).replaceAll('%20', space);
}
