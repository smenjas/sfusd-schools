...
function formatStreet(street) {
    ...
    return name.replace(' Ti St', ' TI St');
}
...
function formatDistance(miles) {
    ...
    return (feet <= 1000) ? `${feet.toFixed(0)} ft.` : `${miles.toFixed(1)} mi.`;
}
...
function sumDistancesBetweenTurns(addressData, jcts, turns, path, newPath, start, end) {
    ...
    console.log(start, path[0], formatDistance(distance), nameCNN(jcts, path[0]));
    ...
    for (let i = 0; i < newPath.length; i++) {
        ...
        if (i === newPath.length - 1) {
            ...
            const lastCNN = newPath[newPath.length - 1];
            distance += howFarAddressToJunction(addressData, jcts, end, lastCNN);
            console.log(lastCNN, end, formatDistance(distance), nameCNN(jcts, lastCNN));
            turns[lastTurn].distance = distance;
        }
        else if (i !== 0 || newPath.length === path.length) {
            ...
            distance += howFarJunctions(jcts, cnn, newPath[i + 1]);
            console.log(cnn, newPath[i + 1], formatDistance(distance), nameCNN(jcts, cnn));
        }
        else {
            console.log('Skipping:', cnn, nameCNN(jcts, cnn));
        }

        if (!(cnn in turns)) {
            // We didn't turn at this intersection: keeping adding distance.
            continue;
        }
...
export function describePath(addressData, jcts, path, start, end) {
    ...
    let html = `<p>Total: ${formatDistance(total)}</p>`;
    ...
    const link = renderDirectionsLink(start, end, 'Google Maps directions');
    html += `<p>${link}</p>`;
