...
function preprocessAddresses(rawAddresses) {
    ...
    Object.entries(rawAddresses).forEach(([street, numbers]) => {
        processed[street] = {};
        Object.entries(numbers).forEach(([number, coords]) => {
...
function preprocessSegments(rawSegments) {
    ...
    Object.entries(rawSegments).forEach(([key, lls]) => {
        const line = [];
        for (const ll of lls) {
            // Convert decimals to full geographic coordinates.
            const [latDec, lonDec] = ll;
            const lat = parseFloat(`37.${latDec}`);
            const lon = parseFloat(`-122.${lonDec}`);
            line.push([lat, lon]);
        }
        processed[key] = { line };
    });
...
function loadMap() {
    ...
    schools = schoolData;
    segments = preprocessSegments(segmentData);
...
function renderBackgroundLayer() {
    ...
    const stats = [
        ...
        `Zoom: ${zoom.toFixed(1)}x, Pan: [${panX.toFixed(0)}, ${panY.toFixed(0)}]`,
        counts.join(', '),
    ].join(' | ');
    log(stats);
