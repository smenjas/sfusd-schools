...
function drawSelectedJunction(x, y, color) {
    if (invisible(x, y)) return;
    const radius = Math.max(6, zoom);
    drawJunction(x, y, radius, color);
...
function drawJunctionStart() {
    ...
    drawSelectedJunction(x, y, getColor('start'));
}
...
function drawJunctionEnd() {
    ...
    drawSelectedJunction(x, y, getColor('end'));
}
...
function drawJunctionLabels() {
    ...
    for (const cnn in junctions) {
        ...
        if (cnnNum === start || cnnNum === end) {
            ctx.font = `${Math.max(18, zoom / 3)}px Arial`;
            ctx.lineWidth = Math.max(3, zoom / 18);
        } else {
            ctx.font = `${Math.max(12, zoom / 4)}px Arial`;
            ctx.lineWidth = Math.max(2, zoom / 24);
        }
        ctx.fillStyle = getColor('text');
        ctx.strokeStyle = getColor('background');
        ctx.miterLimit = 3;
...
function drawMap() {
    ...
    if (zoom >= 25) postprocessAddresses();
